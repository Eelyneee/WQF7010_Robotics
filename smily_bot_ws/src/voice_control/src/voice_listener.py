    ...
    def process_command(self, text_input):
        ...
        with self.mode_lock:
            ...
            if not self.conversation_mode_active:
                ...
                if any(phrase in command_text for phrase in ["take photo", "take a photo", "take a picture", "let's take a picture", "click the shutter"]):
                    self.command_publisher.publish("take_photo")
                elif any(phrase in command_text for phrase in ["view photo", "show me the last photo", "view the picture", "can i see that", "review photos"]):
